...
M.rename = function(_)
    ...
    vim.api.nvim_win_set_cursor(Helper.win, { row, #line })
    Helper.lock_line({ lower = row - 1, upper = row + 1 }, { lower = 2, upper = nil }, nil)
...
M.add = function(args)
    ...
    Helper.lock_line({ lower = row, upper = row + 2 }, { lower = 2, upper = nil }, M.num_lines - 1)
